...
PNG/JPG previews generated by `ImagesMangGenerator` are written with
`origin="lower"`, so the default `--raster-image-origin lower` restores the
array orientation before plotting them next to label maps. Use
`--raster-image-origin upper` only for ordinary raster images that were not
exported with that convention.

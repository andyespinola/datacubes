# Spec: Classifier (v2.2 — reordenamiento por permutación)

> Módulo: `phase_a/classifier.py` · Hito: 2 · Depende de: Extractor

## Cambios v2.2 (2026-07-04) — fix de la inversión bulbo/disco

**El "reordenamiento corregido" de v2.1 (cambio #3 de la tabla de abajo)
resultó ser el origen de una inversión bulbo↔disco en el 23% de la muestra.**
Se reemplaza por asignación conjunta de roles por permutación. Decisión
completa: `docs/adrs/ADR-002-reorder-permutacion-bulbo-disco.md`; evidencia:
`reports/investigacion_inversion_bulbo_disco.md`; impacto en el artículo:
`reports/revision_articulo_IBERAMIA.md`.

| # | Cambio v2.2 | Motivo |
|---|--------|--------|
| A | Reordenamiento **por permutación conjunta** (3! roles) reemplaza la regla secuencial de v2.1 | La regla v2.1 (`disk=argmax ε`) invertía bulbo/disco con empates de ε o pseudo-bulbos rotantes (22/94 galaxias). Ver §"Reordenamiento". |
| B | Bulbo se define por **compacidad + ligadura (SIN ε)**; disco por rotación + extensión | El pseudo-bulbo central rotante debe ir a bulbo (semántica observacional / MORDOR), no a disco. |
| C | Gate de QA radial `radial_inversion_flag` en `run_classifier` | Alerta si `r_bulge > r_disk` (ponderados por prob); habría detectado el 100% de los casos. |
| D | Rama registrada como `reorder_rule_branch="permutation_v2.2"` | Trazabilidad; el feature-set `standard3d` conserva la regla v2.1 (no tiene columnas R/z). |

## Cambios respecto a v2.0 de este spec (histórico v2.1)
...
| 3 | ~~Reordenamiento de componentes "corregido"~~ **ANULADO en v2.2** (cambio A): la regla `disco = ε medio más alto; bulbo = la más ligada del resto` invierte bulbo/disco. | — |
...
### Paso 6 — Reordenamiento de componentes (v2.2: PERMUTACIÓN CONJUNTA)

El GMM no garantiza el orden. **La regla secuencial v2.1 (abajo, conservada
solo como histórico) invierte bulbo↔disco en el 23% de la muestra** cuando el
componente central compacto tiene ε medio comparable/mayor al disco (empate
de ε o pseudo-bulbo rotante). Ver ADR-002 y
`reports/investigacion_inversion_bulbo_disco.md`.

**Regla v2.2 (paper4d):** asignación conjunta de los tres roles por
permutación (3! = 6), puntuando sobre las medias GMM estandarizadas ENTRE
los tres componentes. El bulbo se define por **compacidad + ligadura (sin
ε)** — así el pseudo-bulbo rotante queda en bulbo.

```python
means_orig = scaler.inverse_transform(gmm.means_)   # (3, n_features)
# cols paper4d: 0=ε, 1=log10(R/R_eff+δ), 2=|z|/R_eff, 3=E_norm
m = (means_orig - means_orig.mean(0)) / np.maximum(means_orig.std(0), 1e-9)
s_bulge = -m[:, 1] - m[:, 3]              # compacto (logR bajo) y ligado (E bajo)
s_disk  =  m[:, 0] + m[:, 1] - m[:, 2]    # rotante, extendido, delgado
s_halo  =  m[:, 2] - m[:, 0]             # grueso, no rotante
from itertools import permutations
bulge_k, disk_k, halo_k = max(
    permutations(range(3)),
    key=lambda p: s_bulge[p[0]] + s_disk[p[1]] + s_halo[p[2]])

P_class = P_class[:, [bulge_k, disk_k, halo_k]]   # → [P_bulge, P_disk, P_halo]
```

El feature-set `standard3d` (sin columnas R/z) conserva la regla v2.1.

**Gate de QA:** `run_classifier` calcula el radio medio ponderado por
probabilidad de bulbo y disco y emite `radial_inversion_flag=True` +
warning si `r_bulge > r_disk / 0.9` (bulbo más externo que el disco =
asignación sospechosa).

<details><summary>Regla v2.1 (histórica, ANULADA — invertía bulbo/disco)</summary>

```python
# disk_k = int(np.argmax(means_orig[:, eps_col]))     # ← el argmax arbitrario
# rest = [k for k in range(3) if k != disk_k]         #   con empate de ε
# bulge_k = rest más ligado (E_norm); desempate por log R
# halo_k = restante
```
</details>

Prohibido: cualquier variante que distinga bulbo de halo solo por ε, o que
asigne roles de forma secuencial (el defecto v2.1).
